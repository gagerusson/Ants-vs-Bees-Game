...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            curr_ant = place.ant
            if curr_ant.can_contain(self) and curr_ant.is_container:
                place.ant.store_ant(self)
            elif self.can_contain(curr_ant) and self.is_container:
                self.store_ant(curr_ant)
                place.ant = self 
            else:
    ...
    def buff(self):
        ...
        # BEGIN Problem 12
        if not self.been_buffed:
            self.damage *= 2
        self.been_buffed = True
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        i = 0
        while not curr_place.is_hive and curr_place is not None and i <= self.max_range:
            if curr_place.bees != [] and i >= self.min_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        for b in bee_list:
            b = b.reduce_health(reflective_damage)
        # END Problem 5
